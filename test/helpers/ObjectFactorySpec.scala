...
class ObjectFactorySpec extends Specification {
  ...
  "ObjectFactory" should {
    ...
    "create an empty board" in {
      getTestBoard(TourRequest((0, 0))) must beEqualTo(Board.EMPTY)
    }
    ...
    "create some boards with no blocked squares using the new chunk API" in {
      getOptimisedBoard(TourChunkRequest(TourRequest((5, 5)), Move.O, None)).hasBlockedSquares must beFalse
      getOptimisedBoard(TourChunkRequest(TourRequest((10, 10)), Move.O, None)).hasBlockedSquares must beFalse
    }
